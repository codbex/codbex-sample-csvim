...
export class CityRepository extends Repository<CityEntity> {
    ...
    protected override async triggerEvent(data: EntityEvent<CityEntity>): Promise<void> {
        const triggerExtensions = await Extensions.loadExtensionModules('codbex-sample-csvim-Entities-City', ['trigger']);
        ...
        Producer.topic('codbex-sample-csvim-Entities-City').send(JSON.stringify(data));
    }
}
