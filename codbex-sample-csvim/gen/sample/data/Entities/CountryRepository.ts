...
export class CountryRepository extends Repository<CountryEntity> {
    ...
    protected override async triggerEvent(data: EntityEvent<CountryEntity>): Promise<void> {
        const triggerExtensions = await Extensions.loadExtensionModules('codbex-sample-csvim-Entities-Country', ['trigger']);
        ...
        Producer.topic('codbex-sample-csvim-Entities-Country').send(JSON.stringify(data));
    }
}
