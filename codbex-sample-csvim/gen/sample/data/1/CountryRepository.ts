...
export class CountryRepository extends Repository<CountryEntity> {
    ...
    protected override async triggerEvent(data: EntityEvent<CountryEntity>): Promise<void> {
        const triggerExtensions = await Extensions.loadExtensionModules('codbex-sample-csvim-1-Country', ['trigger']);
        ...
        Producer.topic('codbex-sample-csvim-1-Country').send(JSON.stringify(data));
    }
}
