	...
	.controller('PageController', ($scope, $http, ViewParameters, LocaleService, EntityService) => {
		...
		$scope.create = () => {
			...
			EntityService.create(entity).then((response) => {
				Dialogs.postMessage({ topic: 'codbex-sample-csvim.Entities.Country.entityCreated', data: response.data });
				Notifications.show({
				...
				$scope.$evalAsync(() => {
					$scope.errorMessage = LocaleService.t('codbex-sample-csvim:sample-model.messages.error.unableToCreate', { name: '$t(codbex-sample-csvim:sample-model.t.COUNTRY)', message: message });
				});
		...
		$scope.update = () => {
			...
			EntityService.update(id, entity).then((response) => {
				Dialogs.postMessage({ topic: 'codbex-sample-csvim.Entities.Country.entityUpdated', data: response.data });
				Notifications.show({
					title: LocaleService.t('codbex-sample-csvim:sample-model.t.COUNTRY'),
				...
				const message = error.data ? error.data.message : '';
				$scope.$evalAsync(() => {
					$scope.errorMessage = LocaleService.t('codbex-sample-csvim:sample-model.messages.error.unableToUpdate', { name: '$t(codbex-sample-csvim:sample-model.t.COUNTRY)', message: message });
				});
				console.error('EntityService:', error);
			});
		};
